            ...
            try {
                ...
                if (!up.getFiles().isEmpty()) {
                    if (up.getFiles().size() > 0) {
                        recObj.setImagem1(up.getFiles().get(0));
                        if (up.getFiles().size() > 1) {
                            recObj.setImagem2(up.getFiles().get(1));
                            if (up.getFiles().size() > 2) {
                                recObj.setImagem3(up.getFiles().get(2));
                                if (up.getFiles().size() > 3) {
                                    recObj.setImagem4(up.getFiles().get(3));
                                    if (up.getFiles().size() > 4) {
                                        recObj.setImagem5(up.getFiles().get(4));
                                    }
                                }
                            }
                        }
                ...
                recObj.setNome(up.getForm().get("nome").toString());
                recObj.setDescricao(up.getForm().get("descricao").toString());
                recObj.setServe(Integer.parseInt(up.getForm().get("serve").toString()));
                recObj.setTempoPreparo(d);
                recObj.setAcesso(0);
                recObj.setUsuario(usuario);
                recObj.setCategoria(categoria);
                recObj.setSubcategoria(subCategoria);

                recDAO.incluir(recObj);
                response.sendRedirect("index.jsp?erro=0");
            } catch (Exception e) {

            }
        }else if (request.getParameter("id") != null) {
            recObj = recDAO.buscarPorChavePrimaria(Integer.parseInt(request.getParameter("id")));
        } else {
    ...
    <section id="about" class="three">
        <div class="container">
            <form action="add.jsp" method="post" enctype="multipart/form-data">
                <div class="row">
                    <div class="12u">Id:<input type="text" name="id" value="<%=recObj.getId()%>" readonly /></div>
                    <div class="12u">Nome:<input type="text" name="nome" value="<%=recObj.getNome()%>" required/></div>
                    <div class="12u">Descrição:<textarea name="descricao" value="<%=recObj.getDescricao()%>" required></textarea></div>
